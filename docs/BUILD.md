...
| `capnp-install/capnp_install_stamp` | Stamp file when install succeeded |
| `capnp/` | Generated Cap'n Proto / LStream schema C++ sources |
| `capnp/schemas_built_stamp` | Stamp when LStream schemas were generated |
...
**Qt Creator** uses a shadow build directory such as `build/Desktop_Qt_5_15_2_MinGW_64_bit-Debug/`. The qmake rules call `scripts/mkcapnp.cmd` via the phony make target `capnp_install` (not a path-based target, which clashes with Qt Creator `DESTDIR`). Re-run **qmake** after pulling changes.
...
3. Run tests — in Qt Creator set the run target to **`tests/run_tests.bat`** (not `scripts/run_tests.bat`). That wrapper finds `tests/build/debug/tst_libman_gui.exe`. From a terminal:
   ```cmd
   tests\run_tests.bat
   ```
   Or: `scripts\run_tests.bat path\to\dir\containing\tst_libman_gui.exe`
...
`CMakeLists.txt` runs `scripts/mkcapnp.cmd` (Windows) or `scripts/build_capnp_linux.sh` (Linux) automatically via custom commands that produce `capnp-install/capnp_install_stamp`, then updates LStream schemas via `scripts/mklstream.cmd` (Windows) or `scripts/update_lstream_schemas_linux.sh` (Linux).
